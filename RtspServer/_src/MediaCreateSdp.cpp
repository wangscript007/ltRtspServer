...
MediaCreateSdp::H264CreateSdp(const Buffer &buf, media_stream_ptr SdpMedia, string& SdpTemp)
{
    ...
    snprintf(forchange, 256, "%06X", h264SdpMedia->StreamSps->profile_idc);
	SdpTemp.replace(SdpTemp.find("profile_idc"), 11, forchange);

    snprintf(forchange, 256, "0,0,%d,%d", h264SdpMedia->StreamSps->mb_height, h264SdpMedia->StreamSps->mb_width);
